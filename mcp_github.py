...
def extract_skills_from_state(state: Dict[str, Any]) -> List[str]:
    """
    Extracts candidate and required skills dynamically from state.
    Prioritizes skills generated by the resume and JD analyzer nodes.
    """
    # 1. Prioritize matched skills identified between Candidate and JD
    matched = state.get("matched_skills")
    if matched and isinstance(matched, list):
        return [str(s).strip() for s in matched if s]

    # 2. Fall back to extracted Candidate Resume Skills
    cand_skills = state.get("candidate_skills")
    if cand_skills and isinstance(cand_skills, list):
        return [str(s).strip() for s in cand_skills if s]

    # 3. Check explicit skill keys in state dict
    for key in ["resume_skills", "skills", "extracted_skills", "required_skills"]:
        val = state.get(key)
        if val:
    ...
    # 4. Check nested dictionary structures
...
def check_resume_github_skills_match(resume_skills: List[str], repo_items: List[Dict[str, Any]]) -> str:
    """
    Compares extracted candidate skills against GitHub repo languages,
    topics, descriptions, and repository names.
    """
    if not resume_skills or not repo_items:
        return "> ⚠️ **Resume GitHub Matching Score:** Unable to evaluate skill match due to missing candidate skills in state.\n"
    ...
    if matched_skills:
        ...
        return (
            f"> 💡 **Resume GitHub Matching Score:** **No**, primary candidate skills were not directly detected in GitHub repository titles/descriptions.\n"
        )
